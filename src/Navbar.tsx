  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-3 sm:px-6 pt-3 sm:pt-4 transition-all duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="hidden lg:flex items-center justify-between text-xs text-ink-soft/80 mb-2 px-4 py-1">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5 font-medium text-ink-soft">
              <MapPin className="w-3.5 h-3.5 text-glass" />
              {t.nav.location}
            </span>
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              className="hover:text-brass-dark font-medium transition-colors"
            >
        ...
        <nav
          id="main-nav"
          className={`glass-nav rounded-2xl sm:rounded-full px-3.5 sm:px-6 py-2.5 sm:py-3 transition-all duration-300 ${
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <a
              href="#home"
              className="flex items-center gap-2 sm:gap-3 group focus:outline-none focus-visible:ring-2 focus-visible:ring-brass rounded-lg"
            >
              <div className="w-9 h-9 sm:w-10 sm:h-10 bg-ink flex items-center justify-center text-paper shadow-sm font-display font-semibold tracking-tight text-base sm:text-lg glass-edge">
                AG
              </div>
              <div className="flex flex-col">
                <span className="font-display text-lg sm:text-xl font-semibold tracking-tight text-ink group-hover:text-glass-dark transition-colors">
                  {BUSINESS_INFO.name}
                </span>
                <span className="text-[10px] sm:text-xs text-ink-soft/70 font-medium -mt-0.5 tracking-wide">
                  {t.nav.subtitle}
                </span>
              </div>
            </a>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2.5">
              ...
              <div
                ...
                className="inline-flex items-center bg-paper p-0.5 sm:p-1 rounded-full border border-ink/10"
                role="group"
                aria-label={t.nav.langToggleAria}
              >
                <div className="hidden xs:flex items-center pl-1.5 pr-1 text-ink-soft/50">
              ...
              <button
                ...
                className="md:hidden p-2 rounded-xl text-ink-soft hover:bg-ink/5 focus:outline-none focus:ring-2 focus:ring-brass"
        ...
        {mobileMenuOpen && (
          <div
            id="mobile-nav-menu"
            className="md:hidden mt-2 glass-nav rounded-2xl p-4 shadow-xl animate-in fade-in slide-in-from-top-2 duration-200"
          >
            ...
            <div className="flex items-center justify-between pb-3 mb-3 border-b border-ink/10">
              <span className="text-xs font-semibold text-ink-soft flex items-center gap-1.5">
                <Globe className="w-3.5 h-3.5 text-glass" />
                {t.nav.switchPrompt}
              </span>
              <div className="inline-flex items-center bg-paper p-0.5 rounded-full border border-ink/10">
                <button
                  type="button"
                  onClick={() => setLanguage('en')}
            ...
            <div className="mt-4 pt-4 border-t border-ink/10 flex flex-col gap-2.5 text-xs text-ink-soft">
              ...
              <div className="flex flex-col gap-1.5 pt-1">
                <a
                  href={`tel:+91${BUSINESS_INFO.phones[0].number}`}
                  className="flex items-center gap-2 text-ink font-semibold py-1.5 px-3 bg-paper rounded-lg hover:bg-paper-dim"
                >
                  <Phone className="w-4 h-4 text-brass-dark" />
                  {t.nav.call} {BUSINESS_INFO.phones[0].display}
                </a>
                <a
                  href={`tel:+91${BUSINESS_INFO.phones[1].number}`}
