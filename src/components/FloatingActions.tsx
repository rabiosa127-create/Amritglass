    ...
    <div className="fixed bottom-5 right-4 sm:right-6 z-40 flex flex-col items-end gap-3 pointer-events-none">
      ...
      {showScrollTop && (
        <button
          type="button"
          onClick={scrollToTop}
          className="pointer-events-auto p-2.5 rounded-full bg-white/90 hover:bg-white text-slate-700 hover:text-slate-900 border border-slate-200 shadow-md backdrop-blur-md transition-all active:scale-95 animate-in fade-in slide-in-from-bottom-2"
      ...
      <div className="pointer-events-auto glass-pill rounded-full p-1.5 shadow-lg border border-slate-200/80 flex items-center gap-1.5 bg-white/95">
        
        {/* Call Quick Action */}
        <a
          href={`tel:+91${BUSINESS_INFO.phones[0].number}`}
          className="flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-semibold text-slate-800 hover:bg-slate-100 transition-colors"
